...
Функция: Работа с полным списком покупок
  ...
  Сценарий: изменение количества товаров на панели "Список покупок"
    ...
    Если я нажимаю кнопку "+" 1 раз на товаре "Йогурт Савушкин 2%, 120г"
  ...
  Сценарий: удаление товара на панели "Список покупок"
    ...
    Если я нажимаю кнопку "Удалить" для товара "Йогурт Савушкин 2%, 120г"
    То на панели "Список покупок" должны отобразиться следующие товары:
      |торговая сеть|товарные позиции                     |количество|стоимость без скидки|скидка руб. (%)|стоимость со скидкой|дата окончания акции|
  ...
  Сценарий: добавление комментария в товар на панели "Список покупок"
    ...
    И в строке "Итого" на панели "Список покупок" отображается следующая информация:
      |             |товарные позиции                     |количество|стоимость без скидки|скидка руб. (%)|стоимость со скидкой|
      |Итого:       |      8                              |    26    |       139.17       |  26.51(19 %)  |        112.66 руб. |
    Если я ввожу комментарий "Убедись что свежая!" в поле "Тут можно добавить примечание" на товаре "Булочка Маковая, 100гр"
    То на панели "Список покупок" должны отобразиться следующие товары:
      |торговая сеть|товарные позиции                     |количество|стоимость без скидки|скидка руб. (%)|стоимость со скидкой|дата окончания акции|
      |Виталюр      |      2                              |    6     |       74.06        |  17.00 (23%)  |        57.06       |        18 мая      |
      |             |Булочка Маковая, 100гр               |    2     |       1.30         |  0.20  (15%)  |        1.10        |        18 мая      |
